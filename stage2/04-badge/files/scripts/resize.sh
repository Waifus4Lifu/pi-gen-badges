...
ffmpeg -i ${FILE} -strict -2 -movflags faststart -pix_fmt yuv420p -vf scale=${SCALE} ${TMP}
...
# Then crop...
ffmpeg -i ${TMP} -strict -2 -movflags faststart -pix_fmt yuv420p -filter:v "crop=${CROP}" ${OUT}
# Remove tmp file
rm ${TMP}
